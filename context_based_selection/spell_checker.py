...
def weightedScoreCandidates(cand_words, left_context_words, right_context_words):
    """
    input: a list of words, context sequence
    output: sorted cand words, scores (both in decreasing order)
    """
    selected_cand_words, word_vecs = vocab.getVectors(cand_words)
    _, left_sent_vecs = vocab.getVectors(left_context_words)
    dist_list = range(len(left_sent_vecs))
    _, right_sent_vecs = vocab.getVectors(right_context_words)
    dist_list = dist_list + range(len(right_sent_vecs))
    ...
    if (len(sent_vecs)==0):
        ...
        for dist in range(1, max_dist+1):
            ...
            scores = np.array([getRelevance(subset_sent_vecs, word_vec) for word_vec in word_vecs])
        ...
        if (word not in corpus):# !!! big corpus
            ...
            while (right_ind<word_num and right_num<=context_size):
                if (sent_seq[right_ind] in corpus):
                    ...
                    right_num += 1
                right_ind += 1
            sorted_cand_words, sorted_scores = weightedScoreCandidates(cand_words, left_context_words, right_context_words)
